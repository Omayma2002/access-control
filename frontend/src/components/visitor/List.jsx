...
const List = () => {
    ...
    const fetchVisitors = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5001/api/visitor/${id}`, {
          headers: {
  ...
  return (
    ...
    <div className='flex justify-between items-center'>
      ...
      />
      <Link to="/resident-dashboard/add-visitor" 
      className='px-4 py-1 bg-teal-600 rounded text-white hover:rounded hover:cursor-pointer '>Add New Visitor</Link>
    </div>
    <div className='mt-6'>
      <DataTable 
        columns={columns}
        ...
        // onRowClicked={(row) => {
        //   window.location.href = `/admin-dashboard/visitors/${row._id}`
        // }}
        // pointerOnHover
        // highlightOnHover
      />
