...
const Block = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.post(`http://localhost:5001/api/visitor/block/${id}`, formData, {
        headers: {
          "Authorization": `Bearer ${localStorage.getItem('token')}`,
  ...
  return (
    <div className='max-w-4xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md dark:bg-gray-800 dark:text-white'>
      ...
      <h2 className='text-2xl font-bold mb-6'>Block visitor</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" value={visitor.name} disabled
